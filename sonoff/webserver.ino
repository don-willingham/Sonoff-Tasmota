...
//    strlcpy(Settings.sta_ssid[1], (!strlen(tmp)) ? "" : tmp, sizeof(Settings.sta_ssid[1]));
  ...
  case 5:
    ...
    if (!strlen(tmp) && !strlen(p2) && !strlen(p3)) {
       ...
       return;
    } else if (strncmp(p2, p3, sizeof(Settings.web_password))) {
       snprintf_P(log_data, sizeof(log_data), PSTR("New passwords do not match: %s %s"), p2, p3);
       AddLog(LOG_LEVEL_INFO);
       HandleOtherConfigurationMessage(D_WEB_ADMIN_PASSWORD_DIFF);
       return;
    } else if (strlen(p2) >= sizeof(Settings.web_password)) {
       // If the password was hashed (and maybe salted) length would be less of an issue
       snprintf_P(log_data, sizeof(log_data), PSTR("New password %s is longer than max length of %d"), p2, sizeof(Settings.web_password)-1);
       AddLog(LOG_LEVEL_INFO);
       HandleOtherConfigurationMessage(log_data);
       return;
    } else {
       strlcpy(Settings.web_password, (!strlen(p2)) ? WEB_PASSWORD : (!strcmp(p2,"0")) ? "" : p2, sizeof(Settings.web_password));
    }
#else
